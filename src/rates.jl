...
function rates(;param::parameters,
	...
	n_param  = parameters[parameters(n=n,gamNeg=ngamNeg[i],gL=ngl[i],gH=ngh[i],alTot=nTot[i],alLow=nLow[i],al=afac[i],be=abs(afac[i]/ngamNeg[i]),thetaMidNeutral=θ[i],rho=ρ[i],binom=bn,dac=dac) for i in 1:iterations];
...
# function r_solve(param::parameters,alTot::Float64,alLow::Float64,gH::Int64,gL::Int64,gamNeg::Int64,afac::Float64,θ::Float64,ρ::Float64,e::Vector{Float64})
